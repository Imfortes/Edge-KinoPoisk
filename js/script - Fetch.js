...
function apiSearch(event) {
    ...
    const searchText = document.querySelector(".form-control").value,
        server = "https://api.themoviedb.org/3/search/multi?api_key=c07aed353d8af52093e936f5a0d2d5bc&language=ru&query=" + searchText;
...
function showFullInfo() {
    ...
    if (this.dataset.type === 'movie') {
        url = 'https://api.themoviedb.org/3/movie/' + this.dataset.id + '?api_key=c07aed353d8af52093e936f5a0d2d5bc&language=ru';
    } else if (this.dataset.type === 'tv') {
        url = 'https://api.themoviedb.org/3/tv/' + this.dataset.id + '?api_key=c07aed353d8af52093e936f5a0d2d5bc&language=ru';
    } else {
...
document.addEventListener("DOMContentLoaded", function () {
    fetch('https://api.themoviedb.org/3/trending/all/week?api_key=c07aed353d8af52093e936f5a0d2d5bc&language=ru')
        .then(function (value) {
            if (value.status !== 200)
                return Promise.reject(new Error(value.status));
            return value.json();
...
function getVideo(type, id) {
    ...
    fetch(`https://api.themoviedb.org/3/${type}/${id}/videos?api_key=c07aed353d8af52093e936f5a0d2d5bc&language=ru`)
        .then((value) => {
            if (value.status !== 200){
                return Promise.reject(new Error(value.status));
            }
